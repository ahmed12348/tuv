    ...
    <header class="header-page">
        ...
        <div class="container">
            <div class="header-page-content">
                <h1>{{ $certification->title_en }}</h1>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ url('/') }}">Home</a></li>
                        <li class="breadcrumb-item"><a href="{{ url('certifications') }}">Certifications</a></li>
                        <li class="breadcrumb-item active" aria-current="page">{{ $certification->title_en }}</li>
                    </ol>
                </nav>
            </div>
        </div>
                    ...
                    <div class="blog-details-item desk-pad-left-20">
                        <article class="blog-details-box mb-30">
                            ...
                            <div class="blog-post-image">
                                <img src="{{ asset('storage/' . $certification->image) }}" alt="{{ $certification->title_en }}">
                                <div class="blog-post-category">{{ $certification->title_en }}</div>
                            </div>
                            ...
                            <div class="blog-action mt-4">
                                {{-- You can optionally add tags if available --}}
                                @if (!empty($tags) && count($tags))
                                    <p class="blog-action-tag"><i class="flaticon-tag"></i>
                                        @foreach($tags as $tag)
                                            <a href="{{ url('certifications/tag/' . $tag->id) }}">{{ $tag->title_en }}</a>{{ !$loop->last ? ',' : '' }}
                                        @endforeach
                                    </p>
                                @endif
